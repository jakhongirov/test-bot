...
async function authorize() {
   ...
   if (fs.existsSync(TOKEN_PATH)) {
      ...
      if (!oAuth2Client.credentials.access_token && !oAuth2Client.credentials.refresh_token) {
         throw new Error('Tokens were loaded but access or refresh tokens are missing.');
      }

      console.log("Loaded existing token successfully");
   } else {
      console.log("Token file does not exist, starting new authorization process.");
